...
resource "aws_devicefarm_project" "this" {
  ...
  description                 = try(var.settings.description, "Mobile Device Farm Project - %s", local.farm_name)
...
resource "aws_devicefarm_network_profile" "this" {
  ...
  downlink_bandwidth_bits_per_second = try(each.value.downlink_bandwidth_bits_per_second, null)
  downlink_delay_milliseconds        = try(each.value.downlink_delay_milliseconds, null)
  downlink_jitter_milliseconds       = try(each.value.downlink_jitter_milliseconds, null)
  downlink_packet_loss_percent       = try(each.value.downlink_packet_loss_percent, null)
  uplink_bandwidth_bits_per_second   = try(each.value.uplink_bandwidth_bits_per_second, null)
  uplink_delay_milliseconds          = try(each.value.uplink_delay_milliseconds, null)
  uplink_jitter_milliseconds         = try(each.value.uplink_jitter_milliseconds, null)
  uplink_packet_loss_percent         = try(each.value.uplink_packet_loss_percent, null)
  tags                               = merge(local.all_tags, try(each.value.tags, {}))
}
